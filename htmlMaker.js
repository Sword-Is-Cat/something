...
function newAccountHtml(accName, charaStr){
	...
	let html = 
		'<li id="'+accId+'" class="acc">'+
		'<span>계정명</span>'+
		'<input id="'+accId+'name" type="text" class="input_text" value="'+(accName?accName:'')+'"/>'+
...
function newCharaHtml(charaName, power){
	...
	let html =
		'<li id="'+charaId+'" class="chara">'+
		'<span>캐릭명</span>'+
		'<input id="'+charaId+'name" type="text" class="input_text" value="'+(charaName?charaName:'')+'"/>'+
		'<span>항마력</span>';
	
	for(let i = 0 ; i<textArr.length ; i++)
		html += '<label><input type="radio" name="'+charaId+'po" value="'+valueArr[i]+'" '+(powerArr[i]?'checked':'')+'>'+textArr[i]+'</label>';
...
function buildMemberHtml(member){
	...
	if(member){
		html+='<td>'+(member.name==''?member.charaid:member.name)+'</td>';
		html+='<td>'+(member.power==10?'버스기사':member.power==1?'응애':('1.'+member.power))+'</td>';
